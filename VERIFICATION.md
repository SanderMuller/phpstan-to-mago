...
### The 132/122 pairing is now a test, not a caveat

`phpstan-src-e7` pushed the previous entry one step: two counts of "the same thing" disagreeing is a
*detection method*, and printing scope only helps if someone reads it, whereas an asserted **relationship**
forces the reading. By this log's own fails-when-removed standard that makes it a fix rather than a prompt.

Their guess at the arithmetic was `122 + 8 + 5`; they said the shape mattered more than the figures, and the
shape was right. Derived from the file rather than assumed, the identity that closes is per package:

    verdict lines in the section  ==  headline "portable"  +  headline "registers nowhere"

    symplify 97 == 89 + 8      hihaho 19 == 8 + 11      type-coverage 10 == 10 + 0
    cognitive-complexity 3 == 3 + 0      strict-rules 45 == 45 + 0
    phpunit 13 == 13 + 0      deprecation-rules 2 == 2 + 0

Seven of seven. `test_every_package_headline_accounts_for_every_rule_line()` asserts it, and the caveat that
was prose in my last three reports now lives where a machine meets it.

**Mutation-checked both ways, because a new assertion is exactly the thing this log says not to trust green:**

| mutation | caught by |
|:--|:--|
| headline renders `portable() + 1` | the identity — *"holds 97 rule lines, but its headline accounts for 90 + 8"* |
| headline wording drifts (`registers` → `REGISTERS`) | the parse guard — *"actual size 0 matches expected size 7"* |

The second matters more than the first. Without `assertCount(7, $matches)` a wording change makes the regex
match nothing, the loop runs zero times, and the test passes by never looking — *agreement on zero* in its
purest form, inside the very test written to stop a number drifting.

**What this does and does not check.** The census is generated by this test, so both sides of the identity come
from the same in-memory data: it catches a renderer that lets the headline and the lines disagree, not a
transpiler that misreads a package. That is a narrower claim than the drift alarm beside it, and worth stating
so nobody reads it as the stronger one.

A note on the peer's own accounting, which they volunteered and which I would not have known to ask for: of
their four instrument failures, only two reached me as wrong claims — the other two they caught in-session and
