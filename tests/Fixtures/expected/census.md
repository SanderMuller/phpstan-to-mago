...
checkout has: `--status` counts 258 portable rules here against this file's 208. The other
...
corpus on purpose, so skipping there would be the alarm going green having looked at nothing.
    ...
    symplify/phpstan-rules                    14.15.0
...
## symplify/phpstan-rules — 74 of 105 portable rules the package registers emit, 0 covered by the engine, 31 refuse, 0 unportable in principle, 8 it registers nowhere
...
REFUSE  PreferClassInDefinitionFetchRule
        no node predicate for instanceof PhpParser\Node\Arg on a argument
        needs-at-least: no node predicate for instanceof PhpParser\Node\Arg on a argument
        floor: 1 statement stepped over, so the list above is a floor
...
REFUSE  ServiceStringReferenceCollector
        ...
        floor: 3 statements stepped over, so the list above is a floor
