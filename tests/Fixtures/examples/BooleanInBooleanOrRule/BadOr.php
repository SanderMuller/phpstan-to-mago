...
final class BadOr
{
    /** A non-boolean on the left, which the rule reports as `booleanOr.leftNotBoolean`. */
     ...
     *
     * The reason this file has it rather than only one of the two: the port emits a `report()` per side, or a
     ...
     * *not* the control I first wrote it as. I claimed it separated `||` from `or` and it does not: with
     * this row present the `booleanAnd`/`logicalOr` branch could be a constant and the pair would stay green.
     ...
     *
     * The file is in pint's `notPath`, because `logical_operators` rewrites `or` and the row would go with
     * it.
     */
